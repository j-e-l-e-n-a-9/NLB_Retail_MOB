...
#    And Click on element by text "Add filter"
  ...
  Scenario Outline: Cards-Transactions_Details_[MOB_ANDROID]-Debit_Cards
    ...
    Then Click on element by id "nlb-date"
    And Assert element by text "Amount" has first following sibling with regex "^\d{1,3}(?:\.\d{3})*,\d{2}[\s\u00A0]*RSD$"
    And Assert element by text "Amount in Local Currency" has first following sibling with regex "^\d{1,3}(?:\.\d{3})*,\d{2}[\s\u00A0]*RSD$"
    And Assert element by text "Authorization date" has first following sibling with regex "^\d{2}\.\d{2}\.\d{4}$"
    ...
    And Assert element by text "Transaction ID" has first following sibling with regex "^.+$"
    And Assert element by text "Description" has first following sibling with regex "^.+$"
